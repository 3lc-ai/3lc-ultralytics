...
class TLCSegmentationValidator(TLCDetectionValidator, SegmentationValidator):
    ...
    def postprocess(self, preds: list[torch.Tensor]) -> list[dict[str, torch.Tensor]]:
        """Post-process predictions, mirroring `SegmentationValidator.postprocess`.

        Each image's prototypes and mask coefficients are stashed on the way, so `_scale_filtered_pred` can build
        full-resolution masks for the filtered predictions only.
        """
    ...
    def _compute_3lc_metrics(self, preds, batch):
        annotations = self._process_predictions(preds, batch)

        # Every image in the batch has consumed its stash by now, so release the prototypes and coefficients.
        self._mask_sources = None
        self._mask_imgsz = None

        return {PREDICTED_SEGMENTATIONS: annotations}
