...
The classes trained on are the column's classes other than void, plus the background if the column declares one (it is trained as a class of its own, named `background`). Void pixels are ignored, like Ultralytics' ignore label. A table needs at least two and at most 255 such classes. Ultralytics' binary mode (`nc: 1`, a single output channel) is therefore never used: a column with one class on a background trains a two-class model, and a binary model trained with plain Ultralytics cannot be used to collect metrics on a 3LC table. Metrics collection writes a `mask_predicted` column at the original image resolution, optionally per-sample `ce_loss`, `dice_loss` and `loss` (`collect_loss=True`), and a per-class metrics table with each class' IoU, pixel accuracy and pixel count. Per-sample IoU and pixel accuracy are left to the 3LC Dashboard, which derives them from the ground truth and `mask_predicted` columns.

3LC stores only the id of a column's background class, not its name, so the integration names it `background` in the tables' class names, whatever the table's author called it. Models trained on the tables get these class names, and the per-class metrics table always uses them. A model trained with plain Ultralytics whose name for that class differs is still accepted for metrics collection, in which case Ultralytics' own outputs, such as its confusion matrix plot, show the model's name for it.

Tables created from an Ultralytics dataset YAML record its name in the mask column. Tables created from `cityscapes.yaml` or `cityscapes8.yaml` get Ultralytics' Cityscapes class weights in the cross-entropy loss, both in training and in the per-sample `ce_loss` of metrics collection, whether you train through `data=` or `tables=`. Plain Ultralytics applies them only during training, and only when training through `data=`, so per-sample losses collected on a Cityscapes table after training (or with `model.collect()`) are weighted where Ultralytics' own validation loss on a trained checkpoint is not. Set `cityscapes_class_weights` in `Settings` to `True` to apply the weights to tables that do not record a Cityscapes YAML (they must have Cityscapes' 19 classes), or to `False` to train and collect without them.
